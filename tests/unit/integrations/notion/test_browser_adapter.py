...
async def test_duplicate_page(browser_adapter, fake_browser):
    ...
    # Set up fake browser to return new URL after duplication
...
async def test_create_calendar_view(browser_adapter, fake_browser):
    ...
    result = await browser_adapter.create_calendar_view(
        "db_123",
        "Event Calendar",
        "due_date"
    )
...
async def test_create_board_view(browser_adapter, fake_browser):
    ...
    result = await browser_adapter.create_board_view(
        "db_123",
        "Project Board",
        "status"
    )
