...
@pytest.mark.asyncio
async def test_concurrent_claim_skip_locked(test_database_url: str) -> None:
    ...
    try:
        ...
        async def claim_worker1():
            async with UnitOfWork.from_engine(engine) as uow1:
        ...
        async def claim_worker2():
            async with UnitOfWork.from_engine(engine) as uow2:
                claimed = await claim_ready_job(
                    uow2.session,
        ...
        results = await asyncio.gather(claim_worker1(), claim_worker2())

        # Exactly one worker should succeed
        claimed_jobs = [r for r in results if r is not None]
        assert len(claimed_jobs) == 1, (
            f"Expected exactly one claim to succeed, got {len(claimed_jobs)}"
        )

        # Verify the job is now RUNNING with one worker's lease
        async with UnitOfWork.from_engine(engine) as uow:
