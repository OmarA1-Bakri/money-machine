...
async def test_shortlist_produces_5_candidates(test_workflow, test_database):
    ...
    report_niches = {obs.identity_niche for obs in report.listing_observations if obs.identity_niche}
    for candidate in candidates:
...
async def test_nullable_fields_preserved(test_workflow, test_database):
    ...
    if null_anchor_prices:
        async with AsyncSessionLocal() as session:
            ...
            assert len(null_in_db) > 0, \
                "Expected some observations with NULL anchor_price"
