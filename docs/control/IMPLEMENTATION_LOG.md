...
- **Test coverage expanded**: Browser adapter 35 test functions (53 collected items: 1 parametrized refusal test covering 19 DIRECT_API + COMBINED ops, plus 34 BROWSER operation tests including success/failure/unchanged-ID paths for view creation, both starting states for idempotent toggles, anonymous session usage verification, title read failure, notion.so URL validation). Router 11 test functions (9 existing from base 348161fb + 2 new cache-preservation tests, 11 collected items).
...
- **Test results**: CI on the PR tip: 1035 passed, 1 skipped. Local: browser adapter 35 functions / 53 collected, router 11 functions / 11 collected (9 existing + 2 new), all pass. Zero orchestration/ diffs, api_adapter.py untouched, fixture adapter default preserved.
