...
- **Test coverage expanded**: Browser adapter 31 test functions (49 collected items: 1 parametrized refusal test covering 19 DIRECT_API + COMBINED ops, plus 30 BROWSER operation tests including success/failure paths for ID reading, both starting states for idempotent toggles, anonymous session usage verification, title read failure). Router 11 test functions (11 collected items: 8 existing + 3 new cache-preservation tests).
...
- **Test results**: Full suite 1032 collected, 1031 passed, 1 skipped in CI (run 36060374570 @ 275884f). Local: browser adapter 31 functions / 49 collected, router 11 functions / 11 collected, all pass. Zero orchestration/ diffs, api_adapter.py untouched, fixture adapter default preserved.
