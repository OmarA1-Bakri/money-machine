...
- **Mutation checks** (81 rows; each applied, pytest run, then reverted; 79 killed and 2 equivalent):
...
| Store profiles under the screenshot root | `browser_session.py:29` | `test_browser_profiles_stay_outside_git` |
| Store screenshots under the profile root | `browser_session.py:30` | `test_browser_profiles_stay_outside_git` |
| Drop Zs from profile-name categories | `browser_session.py:32` | `test_profile_name_is_rejected` |
| Allow a 65-character profile name | `browser_session.py:144` | `test_profile_name_is_rejected` |
| Allow a profile name that ends with a hyphen | `browser_session.py:146` | `test_profile_name_is_rejected` |
| Reject a digit in a profile slug | `browser_session.py:153` | `test_profile_slug_is_accepted` |
| Accept an uppercase profile slug | `browser_session.py:153` | `test_profile_name_is_rejected` |
| Skip the authenticated-profile check | `browser_session.py:194` | `test_open_requires_an_authenticated_profile` |
| Compare profile status by its text | `browser_session.py:194` | `test_string_status_is_not_authenticated` |
| Open a new session instead of reusing a healthy one | `browser_session.py:201` | `test_healthy_session_is_reused` |
| Reuse the first open session for every profile | `browser_session.py:196` | `test_two_profiles_are_not_the_same_session` |
| Store a session id that is not a slug | `browser_session.py:173` | `test_open_rejects_a_bad_session_id` |
| Drop publish_page from the operation map | `browser_session.py:53` | `test_mutation_clicks_the_operation_selector` |
| Drop unpublish_page from the operation map | `browser_session.py:54` | `test_mutation_clicks_the_operation_selector` |
| Click the logical name instead of the selector | `browser_session.py:513` | `test_mutation_clicks_the_operation_selector` |
| Skip the idempotency lookup | `browser_session.py:273` | `test_replay_returns_the_same_receipt` |
| Do not store the receipt under its idempotency key | `browser_session.py:483` | `test_replay_returns_the_same_receipt` |
| Look up the idempotency key after requiring an open session | `browser_session.py:273` | `test_replay_after_failure_does_not_click` |
| Skip the captcha check | `browser_session.py:300` | `test_challenge_page_fails_closed[captcha]` |
| Skip the verification check | `browser_session.py:302` | `test_challenge_page_fails_closed[verification]` |
| Click a page that is not normal | `browser_session.py:306` | `test_unknown_page_kind_is_not_clicked` |
| Treat a click timeout as a connection failure | `browser_session.py:345` | `test_click_timeout_is_reconciled` |
| Observe after a connection error | `browser_session.py:347` | `test_connection_error_does_not_observe` |
| Observe after a click runtime error | `browser_session.py:349` | `test_click_runtime_error_is_one_failure_receipt` |
| Click again before reconciling | `browser_session.py:365` | `test_uncertain_click_is_reconciled_when_applied` |
| Record an absent reconciliation as Success | `browser_session.py:386` | `test_uncertain_click_absent_is_a_failure` |
| Record an unknown reconciliation as Success | `browser_session.py:388` | `test_uncertain_click_unknown_stays_unknown` |
| Taint a reconciled applied click | `browser_session.py:383` | `test_uncertain_click_is_reconciled_when_applied` |
| Do not taint a blocked mutation | `browser_session.py:433` | `test_challenge_page_fails_closed[captcha]` |
| Let a screenshot failure skip the receipt | `browser_session.py:447` | `test_screenshot_failure_still_records_one_receipt` |
| Do not append the receipt | `browser_session.py:482` | `test_mutation_clicks_the_operation_selector` |
| Return the receipt list | `browser_session.py:189` | `test_receipts_property_is_a_tuple` |
| Retry a read only once | `browser_session.py:31` | `test_read_retries_timeout_then_returns` |
| Retry a read four times | `browser_session.py:31` | `test_read_stops_after_three_timeouts` |
| Leave a timed-out read healthy | `browser_session.py:437` | `test_read_stops_after_three_timeouts` |
| Retry a read connection error | `browser_session.py:245` | `test_read_does_not_retry_a_connection_error` |
| Read the logical name instead of the selector | `browser_session.py:238` | `test_read_retries_timeout_then_returns` |
| Allow a mutation selector to be read | `browser_session.py:58` | `test_read_rejects_a_mutation_selector` |
| Do not lock the profile when close fails | `browser_session.py:229` | `test_failed_restart_locks_the_profile` |
| Drop the session before close | `browser_session.py:231` | `test_failed_restart_locks_the_profile` |
| Skip the profile bool check | `browser_session.py:115` | `test_profile_status_requires_bool_flags` |
| Allow an absent profile to be authenticated | `browser_session.py:117` | `test_absent_profile_cannot_be_authenticated` |
| Treat an unauthenticated profile as authenticated | `browser_session.py:121` | `test_profile_status_values` |
| Accept a naive timestamp | `browser_session.py:165` | `test_naive_timestamp_is_rejected` |
| Accept a padded mutation token | `browser_session.py:157` | `test_mutation_tokens_must_be_present` |
| Accept an unknown operation | `browser_session.py:511` | `test_unknown_operation_is_rejected` |
| Reuse a tainted session | `browser_session.py:199` | `test_tainted_session_is_not_reused_until_restart` |
| Skip the open-session lock check | `browser_session.py:197` | `test_failed_restart_locks_the_profile` |
| Catch only TimeoutError from observe | `browser_session.py:367` | `test_observe_exception_is_one_unknown_receipt[ConnectionError]` |
| Restore the narrow observe catch | `browser_session.py:367` | `test_observe_exception_is_one_unknown_receipt[RuntimeError]` |
| Skip the page-kind timeout branch | `browser_session.py:288` | `test_page_kind_error_records_one_receipt[timeout]` |
| Skip the page-kind connection branch | `browser_session.py:292` | `test_page_kind_error_records_one_receipt[connection]` |
| Skip the page-kind error branch | `browser_session.py:296` | `test_page_kind_error_records_one_receipt[error]` |
| Drop the locked check in _require_open | `browser_session.py:335` | `test_locked_profile_blocks_mutate_and_read` |
| Pass a garbage click result through | `browser_session.py:352` | `test_unknown_click_result_is_one_error_receipt` |
| Taint only on Failure | `browser_session.py:467` | `test_uncertain_click_unknown_stays_unknown` |
| Unknown page not tainted | `browser_session.py:433` | `test_unknown_page_kind_is_not_clicked` |
| Skip close on a bad session id | `browser_session.py:207` | `test_open_rejects_a_bad_session_id` |
| Let close raise on a bad session id | `browser_session.py:207` | `test_open_rejects_a_bad_session_id_when_close_fails` |
| Accept an idempotency key bound to a different call | `browser_session.py:498` | `test_idempotency_key_is_bound_to_the_call` |
| Drop profile from the idempotency binding | `browser_session.py:499` | `test_idempotency_key_is_bound_to_the_call` |
| Drop operation from the idempotency binding | `browser_session.py:500` | `test_idempotency_key_is_bound_to_the_call` |
| Drop workspace from the idempotency binding | `browser_session.py:501` | `test_idempotency_key_is_bound_to_the_call` |
| Drop target from the idempotency binding | `browser_session.py:502` | `test_idempotency_key_is_bound_to_the_call` |
| Drop job from the idempotency binding | `browser_session.py:503` | `test_idempotency_key_is_bound_to_the_call` |
| Drop profile_path in open_session | `browser_session.py:193` | `test_open_rejects_an_invalid_profile_name` |
| Drop profile_path in restart | `browser_session.py:220` | `test_restart_rejects_an_invalid_profile_name` |
| Drop the operation string check | `browser_session.py:508` | `test_operation_must_be_a_string` |
| Drop the timestamp type check | `browser_session.py:163` | `test_timestamp_must_be_a_datetime` |
| Drop the session-id type check | `browser_session.py:171` | `test_open_rejects_a_non_string_session_id` |
| Drop the screenshot reason check | `browser_session.py:441` | equivalent: every caller reason is already in the screenshot set |
| Record every blocked click as not-clicked | `browser_session.py:419` | `test_connection_error_does_not_observe` |
| Drop session_id from the receipt pre_state | `browser_session.py:474` | `test_mutation_clicks_the_operation_selector` |

- **Control update**: `docs/control/IMPLEMENTATION_STATE.json` `session_06_w9` and this log entry. Against base, STATE changes only these things: `state_revision` 44 to 45, the `session_06_w8` trailing comma, `session_06_w9`, and `updated_at`. `updated_at` moves forward from `2026-09-26T21:21:42Z`. `control_files_and_checkpoint_current` stays false. The session stays incomplete.
- **Hard boundaries**: fixtures and mocks only. No live Notion or Etsy, no real browser, no Playwright import. Fixture adapter stays the default. `src/money_machine/orchestration/` untouched. `uv.lock` untouched. `pyproject.toml` untouched. `src/money_machine/integrations/notion/router.py` untouched. Exit 78 held.
